...
void order_add(){
    ...
    sem_post(semaphores[FIRST_FREE_INDEX]);
...
int main(){
    ...
    while(1){
        ...
        if (getvalue(PACK_NUMBER) + getvalue(SEND_NUMBER) < ORDER_LIMIT)
            order_add();
    }
    return 0;
}