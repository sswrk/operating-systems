...
void matrixesMultiply(int process, int processes, int time_limit, int distinct, int *num_operations){
    ...
    *num_operations = 0;
    ...
    for (int i = 0; i < pairs; i++){
        ...
        int column_zero = process * ceil((double) matrix_B[i]->columns / processes);
        int process_columns = min((process + 1) * (int) ceil((double) matrix_B[i]->columns / processes), matrix_B[i]->columns) - column_zero;
...
int main(int argc, char** argv){
    ...
    int i = 0;
    while(fgets(buffer, 100, f) != NULL) i++;
    ...
    for (int i = 0; i < processes; i++){
        ...
        printf("Proces PID = %d wykonal %d mnozen\n", child_processes[i], WEXITSTATUS(status));
    }
    if (distinct == true){
